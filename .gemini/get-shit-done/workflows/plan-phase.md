...
@/Users/andrejprus/Herd/tenanto/.gemini/get-shit-done/references/ui-brand.md
</required_reading>
...
```bash
INIT=$(node "/Users/andrejprus/Herd/tenanto/.gemini/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "/Users/andrejprus/Herd/tenanto/.gemini/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "/Users/andrejprus/Herd/tenanto/.gemini/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_DESC=$(node "/Users/andrejprus/Herd/tenanto/.gemini/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
**If found:**
1. Read template: `/Users/andrejprus/Herd/tenanto/.gemini/get-shit-done/templates/VALIDATION.md`
...
```bash
UI_PHASE_CFG=$(node "/Users/andrejprus/Herd/tenanto/.gemini/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "/Users/andrejprus/Herd/tenanto/.gemini/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "/Users/andrejprus/Herd/tenanto/.gemini/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
2. Disable Nyquist with the exact command:
   `node "/Users/andrejprus/Herd/tenanto/.gemini/get-shit-done/bin/gsd-tools.cjs" config-set workflow.nyquist_validation false`
