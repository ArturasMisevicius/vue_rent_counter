...
it('renders the audit logs list page as a read-only superadmin surface', function () {
    ...
    $this->actingAs($superadmin)
        ...
        ->assertSeeText('Affected Record Type')
        ->assertSeeText('Date Range')
        ->assertSeeText('From')
        ->assertSeeText('To')
        ...
        ->assertSeeText('Payment Processed')
    ...
    Livewire::test(ListAuditLogs::class)
        ->assertTableColumnExists('actor_summary', fn (TextColumn $column): bool => $column->getLabel() === 'User')
        ->assertTableColumnExists('display_action', fn (TextColumn $column): bool => $column->getLabel() === 'Action')
        ->assertTableColumnExists('record_type_label', fn (TextColumn $column): bool => $column->getLabel() === 'Record Type')
        ->assertTableColumnExists('subject_id', fn (TextColumn $column): bool => $column->getLabel() === 'Record ID')
        ...
        ->assertTableFilterExists('user', fn (Filter $filter): bool => $filter->getLabel() === 'User')
        ->assertTableFilterExists('action_type', fn (SelectFilter $filter): bool => $filter->getLabel() === 'Action Type')
        ->assertTableFilterExists('subject_type', fn (SelectFilter $filter): bool => $filter->getLabel() === 'Affected Record Type')
        ...
        ->assertTableColumnStateSet('display_action', 'Finalized', $auditLog)
        ->assertTableColumnStateSet('record_type_label', 'Invoice', $auditLog)
