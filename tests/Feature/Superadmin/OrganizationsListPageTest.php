...
it('renders the organizations index contract for superadmins', function () {
    ...
    Livewire::test(ListOrganizations::class)
        ->assertTableColumnExists('name', fn (TextColumn $column): bool => $column->getLabel() === 'Organization Name')
        ->assertTableColumnExists('owner.email', fn (TextColumn $column): bool => $column->getLabel() === 'Owner Email')
        ->assertTableColumnExists('currentSubscription.plan', fn (TextColumn $column): bool => $column->getLabel() === 'Plan')
        ->assertTableColumnExists('currentSubscription.status', fn (TextColumn $column): bool => $column->getLabel() === 'Subscription Status')
        ->assertTableColumnExists('buildings_count', fn (TextColumn $column): bool => $column->getLabel() === 'Properties')
        ->assertTableColumnExists('tenants_count', fn (TextColumn $column): bool => $column->getLabel() === 'Tenants')
        ->assertTableColumnExists('created_at', fn (TextColumn $column): bool => $column->getLabel() === 'Created')
        ->assertTableFilterExists('subscription_status', fn (SelectFilter $filter): bool => $filter->getLabel() === 'Subscription Status')
        ->assertTableFilterExists('plan', fn (SelectFilter $filter): bool => $filter->getLabel() === 'Plan')
        ->assertTableFilterExists('created_between', fn (Filter $filter): bool => $filter->getLabel() === 'Created')
