 ...
 * @property-read Collection|User[] $childUsers Tenants created by this Admin
 * @property-read Subscription|null $subscription Subscription (for Admin role)
 * @property-read Collection|Property[] $properties Properties managed by this Admin
 * @property-read Collection|Building[] $buildings Buildings managed by this Admin
 * @property-read Collection|Invoice[] $invoices Invoices for this Admin's organization
 * @property-read Collection|MeterReading[] $meterReadings Meter readings entered by this user
 * @property-read Collection|PersonalAccessToken[] $tokens API tokens for this user
 *
 * @see UserRole
 * @see Subscription
 * @see AccountManagementService
 * @see HierarchicalScope
 */
class User extends Authenticatable implements FilamentUser
{
    ...
    {
        $tenantValue = $tenantId instanceof TenantId ? $tenantId->getValue() : $tenantId;
    ...
    public function assignToTenant(int $tenantId, User $admin): void
    {
        if (! $admin->hasAdministrativePrivileges()) {
            throw new AuthorizationException('Insufficient privileges to assign tenant');
        }
    ...
    {
        if (! $admin->hasAdministrativePrivileges()) {
            throw new AuthorizationException('Insufficient privileges to assign property');
        }
    ...
    {
        if (! $admin->hasAdministrativePrivileges()) {
            throw new AuthorizationException('Insufficient privileges to suspend user');
        }
