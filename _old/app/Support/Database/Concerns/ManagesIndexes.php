 ...
 * Usage in migrations:
 * ```php
 * use App\Support\Database\Concerns\ManagesIndexes;
 *
...
trait ManagesIndexes
{
    ...
    private function validateTableName(string $table): void
    {
        if (! preg_match('/^[a-zA-Z_][a-zA-Z0-9_]*$/', $table)) {
            Log::warning('Invalid table name attempted', [
                'table' => $table,
    ...
    private function validateIndexName(string $index): void
    {
        if (! preg_match('/^[a-zA-Z_][a-zA-Z0-9_]*$/', $index)) {
            Log::warning('Invalid index name attempted', [
                'index' => $index,
                'trace' => debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 3),
            ]);
    ...
    protected function foreignKeyExists(string $table, string $foreignKey): bool
    {
        ...
        try {
            ...
            Log::error('Foreign key check failed', [
                'table' => $table,
                'foreign_key' => $foreignKey,
                'error' => $e->getMessage(),
            ]);
     ...
     *
     * @param  string  $table  The table name
     * @return array<string, Index> Array of index objects keyed by name
     */
