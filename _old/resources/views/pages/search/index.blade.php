...
<div class="container mx-auto px-4 py-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-slate-900">{{ __('shared.search_page.title') }}</h1>
        <p class="text-slate-600 mt-2">{{ __('shared.search_page.description') }}</p>
    </div>
    ...
    <form method="GET" action="{{ route('superadmin.search') }}" class="mb-8">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            <input
                ...
                name="query"
                value="{{ $query }}"
                placeholder="{{ __('shared.search_page.input_placeholder') }}"
    ...
    @if(blank($query))
        <div class="rounded-xl border border-slate-200 bg-white p-6 text-slate-600">
            {{ __('shared.search_page.empty') }}
        </div>
    @else
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div class="rounded-xl border border-slate-200 bg-white p-6">
                <h2 class="text-lg font-semibold text-slate-900 mb-4">{{ __('shared.search_page.organizations') }}</h2>
                @if($organizations->isEmpty())
                    <p class="text-slate-600">{{ __('shared.search_page.no_organizations') }}</p>
                @else
                    <ul class="space-y-2">
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-6">
                <h2 class="text-lg font-semibold text-slate-900 mb-4">{{ __('shared.search_page.users') }}</h2>
                @if($users->isEmpty())
                    <p class="text-slate-600">{{ __('shared.search_page.no_users') }}</p>
                @else
                    <ul class="space-y-2">
