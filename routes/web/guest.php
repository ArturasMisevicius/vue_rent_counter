...
Route::post('/login', [LoginPage::class, 'store'])->name('login.store');
...
Route::post('/forgot-password', [ForgotPasswordPage::class, 'sendResetLink'])->name('password.email');

Route::livewire('/reset-password/{token}', ResetPasswordPage::class)->name('password.reset');
Route::post('/reset-password', [ResetPasswordPage::class, 'resetPassword'])->name('password.update');
