    ...
    private function articleRows(): array
    {
        return [
            ...
            [
                ...
                'body' => "What this feature does\nExplains how a tenant becomes connected to an organization, property, and tenant portal account without admins setting the tenant password manually.\n\nStep-by-step flow\n1. Open Tenants -> Create Tenant.\n2. Enter tenant details and assign the tenant to a property when the property is known.\n3. In Portal Access, choose whether to create portal access, send the invitation immediately, set the preferred portal language, and set the invitation expiration days.\n4. After saving, use the tenant list or tenant profile to see portal access status, invitation status, sent date, accepted date, and last login.\n5. If needed, send, resend, revoke, or copy a fresh invitation link from the tenant profile.\n6. The tenant opens the email or copied link, confirms the organization, tenant profile, property, and email, then creates a password.\n7. Acceptance activates the tenant account, verifies email, enables portal access, and redirects the tenant into the portal.\n\nImportant rules\nInvitation links are single-use and expire. Raw tokens are never stored, so copying a link creates a fresh token and revokes older pending invitations. Inactive, moved-out, or archived tenants cannot accept old invitations. Disabled portal access blocks tenant portal access without deleting invoices, readings, documents, or audit history.\n\nTroubleshooting\nIf the link expired or the tenant never received the email, resend the invitation from the tenant profile. If the tenant should not access the portal anymore, disable portal access. If an admin or manager cannot send an invitation, check organization scope and manager tenant permissions.",
                'locale' => 'en',
                'role' => HelpAudienceRole::ADMIN->value,
