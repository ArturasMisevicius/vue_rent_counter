...
- **Global scope**: `/Users/andrejprus/Herd/tenanto/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/andrejprus/Herd/tenanto/.claude/notes/`)
...
2. Glob `/Users/andrejprus/Herd/tenanto/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/andrejprus/Herd/tenanto/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/andrejprus/Herd/tenanto/.claude/notes/` — works in any directory
