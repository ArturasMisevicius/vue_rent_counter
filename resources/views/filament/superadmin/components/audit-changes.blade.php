                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @if(isset($value['old']))
                                    <div class="bg-red-50 rounded border border-red-200 p-3">
                                        <div class="text-xs font-medium text-red-800 mb-1">{{ __('superadmin.audit.values.before') }}</div>
                                        <div class="text-sm text-red-700">
                                            ...
                                            @else
                                                {{ $value['old'] ?? __('superadmin.audit.values.empty') }}
                                            @endif
                                        </div>
                                    </div>
                                @endif
                                ...
                                @if(isset($value['new']))
                                    <div class="bg-green-50 rounded border border-green-200 p-3">
                                        <div class="text-xs font-medium text-green-800 mb-1">{{ __('superadmin.audit.values.after') }}</div>
                                        <div class="text-sm text-green-700">
                                            ...
                                            @else
                                                {{ $value['new'] ?? __('superadmin.audit.values.empty') }}
                                            @endif
                                        </div>
                                    </div>
                                @endif
                    ...
                    @else
                        ...
                        <div class="bg-blue-50 rounded border border-blue-200 p-3">
                            <div class="text-sm text-blue-700">
                                {{ $value ?? __('superadmin.audit.values.empty') }}
                            </div>
                        </div>
                    @endif
                </div>
...
@else
    <div class="text-center py-8 text-gray-500">
        ...
        <p class="mt-2 text-sm">{{ __('superadmin.audit.values.no_changes') }}</p>
    </div>
@endif