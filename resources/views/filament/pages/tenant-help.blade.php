<x-filament-panels::page>
    <div class="space-y-6">
        <section class="border-b border-slate-200 pb-6">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div class="max-w-3xl space-y-2">
                    <p class="text-sm font-semibold text-emerald-700">{{ __('help.pages.tenant.eyebrow') }}</p>
                    <h2 class="text-3xl font-semibold tracking-tight text-slate-950">{{ __('help.pages.tenant.heading') }}</h2>
                    <p class="text-sm leading-6 text-slate-600">{{ __('help.pages.tenant.description') }}</p>
                </div>
        ...
        <section class="flex flex-wrap gap-2">
            <a
                href="{{ route('filament.admin.pages.tenant-help') }}"
                wire:navigate
            ...
            @forelse ($this->categories as $option)
                <a
                    href="{{ route('filament.admin.pages.tenant-help', ['category' => $option['value']]) }}"
                    wire:navigate
