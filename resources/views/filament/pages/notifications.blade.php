<x-filament-panels::page>
    <div class="space-y-6">
        <section class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            <p class="text-sm font-semibold uppercase tracking-[0.24em] text-amber-600">{{ __('notifications.center.eyebrow') }}</p>
            <h1 class="mt-3 text-3xl font-semibold tracking-tight text-slate-950">{{ __('notifications.center.title') }}</h1>
            <p class="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
                {{ __('notifications.center.description') }}
            </p>
        </section>

        @if (filled($statusMessage))
            <section class="rounded-lg border border-emerald-200 bg-emerald-50 px-5 py-4 text-sm font-medium text-emerald-700">
                {{ $statusMessage }}
            </section>
        @endif
                ...
                <div class="flex items-start justify-between gap-4">
                    ...
                    @if ($unreadCount > 0)
                        <button
                            ...
                            class="inline-flex items-center justify-center rounded-md border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50"
                        >
        ...
        <section class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <div class="grid gap-3 md:grid-cols-2">
                <label class="flex flex-col gap-2">
                    <span class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{{ __('notifications.center.filters.type') }}</span>
                    <select wire:model.live="typeFilter" class="rounded-md border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-700">
                        <option value="">{{ __('notifications.center.filters.all_types') }}</option>
                ...
                <label class="flex flex-col gap-2">
                    <span class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{{ __('notifications.center.filters.status') }}</span>
                    <select wire:model.live="statusFilter" class="rounded-md border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-700">
            ...
            @if (count($notifications) > 0)
                <div class="divide-y divide-slate-100">
                    @foreach ($notifications as $notification)
                        <article wire:key="notification-center-{{ $notification['id'] }}" class="flex flex-col gap-4 px-6 py-5 lg:flex-row lg:items-start lg:justify-between">
