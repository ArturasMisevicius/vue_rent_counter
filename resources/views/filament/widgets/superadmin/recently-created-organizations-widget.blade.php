    ...
    <div class="space-y-3">
        @forelse ($organizations as $organization)
            <article class="rounded-2xl border border-slate-200 bg-slate-50/70 px-4 py-4">
                ...
                <p class="mt-1 text-sm text-slate-600">{{ $organization->created_at?->locale(app()->getLocale())->translatedFormat(\App\Filament\Support\Formatting\LocalizedDateFormatter::dateFormat()) }}</p>
            </article>
        @empty
