    ...
    <x-tenant.section-card :title="__('invoices.shared.receipt.details')">
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div class="text-left sm:text-right">
                <p class="text-sm text-slate-600">{{ __('invoices.shared.receipt.payment_status') }}</p>
                <p class="text-lg font-semibold text-slate-900">
                    {{ $invoice->isPaid() ? __('invoices.shared.receipt.paid') : enum_label($invoice->status) }}
                </p>
            </div>
            <div>
                <p class="text-sm text-slate-600">{{ __('invoices.shared.receipt.period') }}</p>
                <p class="text-sm text-slate-900">
                    {{ $invoice->billing_period_start->format('Y-m-d') }} - {{ $invoice->billing_period_end->format('Y-m-d') }}
                </p>
            </div>
            <div class="text-left sm:text-right">
                <p class="text-sm text-slate-600">{{ __('invoices.shared.receipt.total') }}</p>
                <p class="text-2xl font-semibold text-slate-900">€{{ number_format($invoice->total_amount, 2) }}</p>
            </div>
            <div>
                <p class="text-sm text-slate-600">{{ __('invoices.shared.receipt.paid_at') }}</p>
                <p class="text-sm text-slate-900">{{ $invoice->paid_at?->format('Y-m-d H:i') ?? '—' }}</p>
            </div>
            <div>
                <p class="text-sm text-slate-600">{{ __('invoices.shared.receipt.payment_reference') }}</p>
                <p class="text-sm text-slate-900">{{ $invoice->payment_reference ?? '—' }}</p>
            </div>
            <div>
                <p class="text-sm text-slate-600">{{ __('invoices.shared.receipt.paid_amount') }}</p>
                <p class="text-sm text-slate-900">
            ...
            @if($invoice->due_date)
            <div>
                <p class="text-sm text-slate-600">{{ __('invoices.shared.receipt.due_date') }}</p>
                <p class="text-sm text-slate-900">{{ $invoice->due_date->format('Y-m-d') }}</p>
            </div>
            @endif
            @if($invoice->tenant && $invoice->tenant->property)
            <div class="sm:col-span-2">
                <p class="text-sm text-slate-600">{{ __('invoices.shared.receipt.property') }}</p>
    ...
    <x-tenant.section-card :title="__('invoices.shared.receipt.line_items')">
        <div class="hidden sm:block overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        <th class="px-3 py-3 text-left text-xs font-semibold uppercase tracking-[0.12em] text-slate-600">{{ __('invoices.shared.receipt.description') }}</th>
                        <th class="px-3 py-3 text-right text-xs font-semibold uppercase tracking-[0.12em] text-slate-600">{{ __('invoices.shared.receipt.qty') }}</th>
                        <th class="px-3 py-3 text-right text-xs font-semibold uppercase tracking-[0.12em] text-slate-600">{{ __('invoices.shared.receipt.unit_price') }}</th>
                        <th class="px-3 py-3 text-right text-xs font-semibold uppercase tracking-[0.12em] text-slate-600">{{ __('invoices.shared.receipt.sum') }}</th>
                    </tr>
                </thead>
        ...
        <x-tenant.stack gap="3" class="sm:hidden">
            ...
            <div class="rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
                ...
                <p class="text-xs text-slate-600">{{ __('invoices.shared.receipt.qty_short') }} {{ number_format($item->quantity, 2) }}</p>
                <p class="text-xs text-slate-600">{{ __('invoices.shared.receipt.unit_short') }} €{{ number_format($item->unit_price, 4) }}</p>
            ...
            <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 shadow-inner">
                <div class="flex items-center justify-between">
                    <p class="text-sm font-semibold text-slate-900">{{ __('invoices.shared.receipt.total_sum') }}</p>
