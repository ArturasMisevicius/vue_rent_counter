...
@section('title', __('reports.manager.index.title'))

@section('content')
<div class="px-4 sm:px-6 lg:px-8">
<div class="sm:flex sm:items-center sm:justify-between">
        <div class="sm:flex-auto">
            <h1 class="text-2xl font-semibold text-slate-900">{{ __('reports.manager.index.title') }}</h1>
            <p class="mt-2 text-sm text-slate-700">{{ __('reports.manager.index.description') }}</p>
        </div>
    </div>
    ...
    <div class="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        <x-stat-card>
            ...
            <x-slot name="label">{{ __('reports.manager.index.stats.properties') }}</x-slot>
        ...
        <x-stat-card>
            ...
            <x-slot name="label">{{ __('reports.manager.index.stats.meters') }}</x-slot>
        ...
        <x-stat-card>
            ...
            <x-slot name="label">{{ __('reports.manager.index.stats.readings') }}</x-slot>
        ...
        <x-stat-card>
            ...
            <x-slot name="label">{{ __('reports.manager.index.stats.invoices') }}</x-slot>
    ...
    <div class="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <a href="{{ route('manager.reports.consumption') }}" class="group relative overflow-hidden rounded-2xl border border-indigo-100 bg-gradient-to-br from-indigo-50 via-white to-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
            <div class="flex items-start gap-3">
                ...
                <div>
                    <p class="text-sm font-semibold text-slate-900">{{ __('reports.manager.index.cards.consumption.title') }}</p>
                    <p class="mt-1 text-xs text-slate-600">{{ __('reports.manager.index.cards.consumption.description') }}</p>
                </div>
            </div>
            <div class="mt-4 inline-flex items-center gap-2 text-xs font-semibold text-indigo-700">
                <span class="h-2 w-2 rounded-full bg-indigo-500"></span>
                {{ __('reports.manager.index.cards.consumption.cta') }}
            </div>
        </a>
        ...
        <a href="{{ route('manager.reports.revenue') }}" class="group relative overflow-hidden rounded-2xl border border-slate-100 bg-gradient-to-br from-slate-50 via-white to-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
            <div class="flex items-start gap-3">
                ...
                <div>
                    <p class="text-sm font-semibold text-slate-900">{{ __('reports.manager.index.cards.revenue.title') }}</p>
                    <p class="mt-1 text-xs text-slate-600">{{ __('reports.manager.index.cards.revenue.description') }}</p>
                </div>
            </div>
            <div class="mt-4 inline-flex items-center gap-2 text-xs font-semibold text-slate-700">
                <span class="h-2 w-2 rounded-full bg-emerald-400"></span>
                {{ __('reports.manager.index.cards.revenue.cta') }}
            </div>
        </a>
        ...
        <a href="{{ route('manager.reports.meter-reading-compliance') }}" class="group relative overflow-hidden rounded-2xl border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
            <div class="flex items-start gap-3">
                ...
                <div>
                    <p class="text-sm font-semibold text-slate-900">{{ __('reports.manager.index.cards.compliance.title') }}</p>
                    <p class="mt-1 text-xs text-slate-600">{{ __('reports.manager.index.cards.compliance.description') }}</p>
                </div>
            </div>
            <div class="mt-4 inline-flex items-center gap-2 text-xs font-semibold text-emerald-700">
                <span class="h-2 w-2 rounded-full bg-emerald-500"></span>
                {{ __('reports.manager.index.cards.compliance.cta') }}
            </div>
        </a>
    </div>
    ...
    <div class="mt-8">
        <x-card>
            <x-slot name="title">{{ __('reports.manager.index.guide.title') }}</x-slot>
            
            <div class="mt-4 space-y-3 text-sm text-slate-600">
                <p><strong class="text-slate-900">{{ __('reports.manager.index.guide.items.consumption.title') }}:</strong> {{ __('reports.manager.index.guide.items.consumption.body') }}</p>
                <p><strong class="text-slate-900">{{ __('reports.manager.index.guide.items.revenue.title') }}:</strong> {{ __('reports.manager.index.guide.items.revenue.body') }}</p>
                <p><strong class="text-slate-900">{{ __('reports.manager.index.guide.items.compliance.title') }}:</strong> {{ __('reports.manager.index.guide.items.compliance.body') }}</p>
            </div>
        </x-card>
    </div>
</div>
