...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-slate-900">{{ __('organizations.pages.create.title') }}</h1>
        </div>
                ...
                <div>
                    ...
                    <select class="w-full px-3 py-2 border border-slate-300 rounded" id="plan" name="plan" required>
                        @foreach(\App\Enums\SubscriptionPlan::cases() as $plan)
                            <option value="{{ $plan->value }}" @selected(old('plan') === $plan->value)>{{ enum_label($plan) }}</option>
                        @endforeach
                    </select>
                </div>
