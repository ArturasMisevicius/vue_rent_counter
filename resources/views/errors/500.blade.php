...
<head>
    ...
    <title>{{ __('error_pages.500.title') }}</title>
        ...
        <div class="max-w-md w-full space-y-8 text-center">
            <div>
                ...
                <h2 class="mt-6 text-3xl font-extrabold text-slate-900">
                    {{ __('error_pages.500.headline') }}
                </h2>
                <p class="mt-2 text-sm text-slate-600">
                    {{ __('error_pages.500.description') }}
                </p>
            </div>
            
            <div class="bg-white shadow rounded-lg p-6 text-left">
                <h3 class="text-sm font-medium text-slate-900 mb-2">{{ __('error_pages.common.actions_title') }}</h3>
                <ul class="space-y-2 text-sm text-slate-600">
                    <li class="flex items-start">
                        ...
                        {{ __('error_pages.common.try_refresh') }}
                    </li>
                    <li class="flex items-start">
                        <svg class="h-5 w-5 text-indigo-500 mr-2 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                        </svg>
                        {{ __('error_pages.common.try_again') }}
                    </li>
                    <li class="flex items-start">
                        <svg class="h-5 w-5 text-indigo-500 mr-2 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                        </svg>
                        {{ __('error_pages.common.contact_support') }}
                    </li>
                </ul>
            </div>
