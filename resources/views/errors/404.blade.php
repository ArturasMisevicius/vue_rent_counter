...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ __('shell.error_404_title') }} · {{ config('app.name', 'Tenanto') }}</title>
    ...
    <body class="min-h-screen bg-brand-ink text-slate-950 antialiased">
        <div class="relative isolate min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top,rgba(62,197,173,0.16),transparent_28%),linear-gradient(180deg,#fff8eb_0%,#f8f4ea_50%,#eef7f5_100%)]">
            <main class="relative flex min-h-screen items-center justify-center px-6 py-12">
                <div class="w-full max-w-2xl space-y-8">
                    ...
                    <section class="rounded-[2rem] border border-white/70 bg-white/92 p-8 text-center shadow-[0_28px_90px_rgba(15,23,42,0.14)] backdrop-blur xl:p-10">
                        ...
                        <h1 class="mt-4 font-display text-4xl tracking-tight text-slate-950">{{ __('shell.error_404_title') }}</h1>
                        <p class="mx-auto mt-4 max-w-xl text-sm leading-6 text-slate-600">{{ __('shell.error_404_message') }}</p>
