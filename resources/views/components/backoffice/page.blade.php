            ...
            <div class="relative px-5 py-6 sm:px-8 sm:py-8">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div class="ds-shell__header">
                        ...
                        @endif
                        <h1 class="ds-shell__title mt-1 text-2xl font-semibold text-slate-900 sm:text-3xl">{{ $title }}</h1>
                        @if($description)
                            <p class="ds-shell__description mt-2 max-w-3xl text-sm text-slate-600 sm:text-base">{{ $description }}</p>
                        @endif
                    </div>
                    @isset($actions)
