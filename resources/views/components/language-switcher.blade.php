...
@if($languageData->isNotEmpty())
    @if($variant === 'select')
        <div class="{{ $getContainerClasses() }}">
            <form method="GET" action="{{ route('language.switch', ['locale' => $currentLocale]) }}" class="{{ $getFormClasses() }}">
                ...
                <select
                    ...
                    class="{{ $getBaseClasses() }} block w-full rounded-md border-0 py-1.5 pl-3 pr-10 ring-1 ring-inset ring-white/20 focus:ring-2 focus:ring-inset sm:text-sm sm:leading-6"
    ...
    @else
        {{-- Dropdown variant --}}
        <div class="{{ $getContainerClasses() }}" x-data="{ open: false }">
            <button
                @click="open = !open"
                @keydown.escape="open = false"
                class="{{ $getBaseClasses() }} inline-flex items-center gap-x-1.5 rounded-md px-3 py-2 text-sm font-semibold text-white shadow-sm ring-1 ring-inset ring-white/20 hover:bg-white/20"
                aria-expanded="false"
                aria-haspopup="true"
                aria-label="{{ __('common.select_language') }}"
                type="button"
            ...
            <div
                ...
                class="{{ $getDropdownMenuClasses() }} origin-top-right focus:outline-none"
                role="menu"
            ...
            >
                @foreach($languageData as $language)
                    <a
                        ...
                        class="{{ $getDropdownItemClasses($language) }}"
                        role="menuitem"
                        @click="open = false"
...
@else
    ...
    <div class="{{ $getContainerClasses() }}" role="status" aria-live="polite">
