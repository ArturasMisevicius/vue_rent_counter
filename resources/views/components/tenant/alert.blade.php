...
<div {{ $attributes->merge(['class' => 'relative overflow-hidden rounded-xl border shadow-sm ' . match($type) {
    'warning' => 'border-amber-200 bg-amber-50',
    'error' => 'border-rose-200 bg-rose-50',
    'success' => 'border-emerald-200 bg-emerald-50',
    default => 'border-sky-200 bg-sky-50',
}]) }}>
    <div class="absolute left-0 top-0 h-full w-1 {{ match($type) {
        'warning' => 'bg-amber-400',
        'error' => 'bg-rose-500',
        'success' => 'bg-emerald-500',
        default => 'bg-sky-500',
    } }}"></div>
    <div class="flex gap-3 p-4 sm:p-5 {{ match($type) {
