...
<div class="relative overflow-hidden min-h-screen">
    ...
    <header class="relative max-w-6xl mx-auto px-6 pt-10 flex items-center justify-between">
        ...
        @if($canLogin)
            <div class="flex items-center gap-3">
                ...
                @if($canRegister)
                    <a href="{{ route('register') }}" class="inline-flex items-center gap-2 rounded-xl bg-white text-slate-900 px-4 py-2 text-sm font-semibold shadow-lg shadow-white/20 transition">
                        {{ __('app.cta.register') }}
    ...
    <main class="relative max-w-6xl mx-auto px-6 pb-16">
        <section class="grid lg:grid-cols-2 gap-12 pt-16">
            <div class="space-y-6">
                ...
                <div class="flex flex-wrap gap-3">
                    @if($canLogin)
                        <a href="{{ route('login') }}" class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-500 to-sky-500 px-5 py-3 text-sm font-semibold text-white shadow-glow transition">
                    ...
                    @endif
                    @if($canRegister)
                        <a href="{{ route('register') }}" class="inline-flex items-center gap-2 rounded-xl border border-white/20 px-5 py-3 text-sm font-semibold text-white/90 backdrop-blur transition hover:border-white/40">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm text-slate-300">
                    <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                        <p class="text-2xl font-display text-white">{{ __('landing.metric_values.five_minutes') }}</p>
                        <p class="text-slate-400">{{ __('landing.metrics.cache') }}</p>
                    </div>
                    <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                        <p class="text-2xl font-display text-white">{{ __('landing.metric_values.zero') }}</p>
                        <p class="text-slate-400">{{ __('landing.metrics.readings') }}</p>
                    </div>
                    <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                        <p class="text-2xl font-display text-white">{{ __('landing.metric_values.full') }}</p>
            ...
            <div class="relative">
                ...
                <div class="relative rounded-3xl border border-white/10 bg-white/5 p-6 shadow-[0_24px_80px_rgba(0,0,0,0.35)] backdrop-blur">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs uppercase tracking-[0.2em] text-slate-300">{{ __('landing.dashboard.live_overview') }}</p>
                            <p class="mt-2 text-xl font-display text-white">{{ __('landing.dashboard.portfolio_health') }}</p>
                        </div>
                    ...
                    <div class="mt-6 grid grid-cols-2 gap-4">
                        ...
                        <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                            <p class="text-sm text-slate-300">{{ __('landing.dashboard.meters_validated') }}</p>
                            <p class="mt-1 text-3xl font-display text-white">98%</p>
                            <p class="text-xs text-slate-400">{{ __('landing.dashboard.meters_validated_hint') }}</p>
                        </div>
                        <div class="col-span-2 rounded-2xl border border-white/10 bg-white/5 p-4">
                            <p class="text-sm text-slate-300 mb-2">{{ __('landing.dashboard.recent_readings') }}</p>
                            <div class="grid grid-cols-3 gap-3 text-xs text-slate-300">
                                <div class="rounded-xl bg-white/5 px-3 py-2 border border-white/5">
                                    <p class="font-semibold text-white">{{ __('landing.dashboard.water') }}</p>
                                    <p class="text-slate-400">{{ __('landing.dashboard.water_status') }}</p>
                                </div>
                                <div class="rounded-xl bg-white/5 px-3 py-2 border border-white/5">
                                    <p class="font-semibold text-white">{{ __('landing.dashboard.electricity') }}</p>
                                    <p class="text-slate-400">{{ __('landing.dashboard.electricity_status') }}</p>
                                </div>
                                <div class="rounded-xl bg-white/5 px-3 py-2 border border-white/5">
                                    <p class="font-semibold text-white">{{ __('landing.dashboard.heating') }}</p>
                                    <p class="text-slate-400">{{ __('landing.dashboard.heating_status') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                @foreach($features as $feature)
                    <div class="group relative rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur transition hover:border-white/20">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-xs font-semibold text-slate-400">{{ __('landing.dashboard.trusted') }}</span>
                        </div>
        ...
        <section id="faq" class="mt-20 grid lg:grid-cols-2 gap-10">
            <div class="space-y-3">
                <p class="text-xs uppercase tracking-[0.22em] text-slate-400">{{ __('landing.faq_section.eyebrow') }}</p>
                <h2 class="text-3xl font-display font-bold text-white">{{ __('landing.faq_section.title') }}</h2>
                ...
                @foreach($faqItems as $faq)
                    <details class="group rounded-2xl border border-white/10 bg-white/5 px-5 py-4 backdrop-blur">
                        ...
                        @if(!empty($faq['category']))
                            <p class="mt-2 text-xs text-slate-400">{{ __('landing.faq_section.category_prefix') }} {{ __($faq['category']) }}</p>
                        @endif
                    </details>
                @endforeach
            </div>
