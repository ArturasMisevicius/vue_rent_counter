...
@section('title', __('reports.manager.compliance.title'))

@section('content')
<div class="px-4 sm:px-6 lg:px-8">
<div class="sm:flex sm:items-center sm:justify-between">
        <div class="sm:flex-auto">
            <h1 class="text-2xl font-semibold text-slate-900">{{ __('reports.manager.compliance.title') }}</h1>
            <p class="mt-2 text-sm text-slate-700">{{ __('reports.manager.compliance.description') }}</p>
        </div>
        <div class="mt-4 sm:ml-16 sm:mt-0 sm:flex-none">
            <form method="GET" action="{{ route('manager.reports.compliance.export') }}" class="inline">
                ...
                <x-button type="submit" variant="secondary">
                    ...
                    {{ __('reports.manager.compliance.export') }}
                </x-button>
            </form>
        </div>
    </div>
    ...
    <div class="mt-6">
        <x-card>
            <form method="GET" action="{{ route('manager.reports.meter-reading-compliance') }}" class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <x-form-input
                    name="month"
                    label="{{ __('reports.manager.compliance.filters.month') }}"
                    type="month"
                ...
                <x-form-select
                    name="building_id"
                    label="{{ __('reports.manager.compliance.filters.building') }}"
                    :options="$buildings->pluck('name', 'id')->toArray()"
                    :selected="$buildingId"
                    placeholder="{{ __('reports.manager.compliance.filters.placeholders.building') }}"
                />

                <div class="flex items-end">
                    <x-button type="submit" class="w-full">
                        {{ __('reports.manager.compliance.filters.submit') }}
                    </x-button>
                </div>
            </form>
        </x-card>
    ...
    <div class="mt-8">
        <x-card>
            <x-slot name="title">{{ __('reports.manager.compliance.summary.title') }}</x-slot>
            
            <div class="mt-4">
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-3">
                    <div class="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3">
                        <p class="text-sm text-emerald-700">{{ __('reports.manager.compliance.summary.complete.label') }}</p>
                        <p class="mt-1 text-2xl font-semibold text-emerald-900">{{ $propertiesWithReadings->count() }}</p>
                        <p class="text-xs text-emerald-600">{{ __('reports.manager.compliance.summary.complete.description') }}</p>
                    </div>
                    <div class="rounded-lg border border-yellow-200 bg-yellow-50 px-4 py-3">
                        <p class="text-sm text-yellow-700">{{ __('reports.manager.compliance.summary.partial.label') }}</p>
                        <p class="mt-1 text-2xl font-semibold text-yellow-900">{{ $propertiesWithPartialReadings->count() }}</p>
                        <p class="text-xs text-yellow-600">{{ __('reports.manager.compliance.summary.partial.description') }}</p>
                    </div>
                    <div class="rounded-lg border border-red-200 bg-red-50 px-4 py-3">
                        <p class="text-sm text-red-700">{{ __('reports.manager.compliance.summary.none.label') }}</p>
                        <p class="mt-1 text-2xl font-semibold text-red-900">{{ $propertiesWithNoReadings->count() }}</p>
                        <p class="text-xs text-red-600">{{ __('reports.manager.compliance.summary.none.description') }}</p>
                    </div>
                </div>
                ...
                <div class="mt-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm text-slate-500">{{ __('reports.manager.compliance.summary.overall') }}</p>
                        ...
                        <div class="text-right">
                            <p class="text-sm text-slate-500">{{ __('reports.manager.compliance.summary.properties') }}</p>
    ...
    <div class="mt-8">
        <x-card>
            <x-slot name="title">{{ __('reports.manager.compliance.by_building.title') }}</x-slot>
                    ...
                    <div class="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-xs text-slate-600">{{ __('reports.manager.compliance.by_building.properties', ['complete' => $data['complete'], 'total' => $data['total']]) }}</p>
                            </div>
                            <div class="text-right">
    ...
    <div class="mt-8">
        <x-card>
            <x-slot name="title">{{ __('reports.manager.compliance.by_meter_type.title') }}</x-slot>
            ...
            <div class="mt-4">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div class="rounded-lg border border-slate-200 bg-white px-4 py-3">
                        ...
                        <p class="text-xs text-slate-600">{{ __('reports.manager.compliance.by_meter_type.meters', ['complete' => $data['complete'], 'total' => $data['total']]) }}</p>
                    </div>
                    @endforeach
                </div>
            </div>
    ...
    <div class="mt-8">
        <x-card>
            <x-slot name="title">{{ __('reports.manager.compliance.details.title') }}</x-slot>
            
            <div class="mt-4">
                <div class="hidden sm:block">
                <x-data-table caption="{{ __('reports.manager.compliance.details.caption') }}">
                    <x-slot name="header">
                        <tr>
                            <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-slate-900 sm:pl-0">{{ __('reports.manager.compliance.details.headers.property') }}</th>
                            <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-slate-900">{{ __('reports.manager.compliance.details.headers.building') }}</th>
                            <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-slate-900">{{ __('reports.manager.compliance.details.headers.total_meters') }}</th>
                            <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-slate-900">{{ __('reports.manager.compliance.details.headers.readings_submitted') }}</th>
                            <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-slate-900">{{ __('reports.manager.compliance.details.headers.status') }}</th>
                            <th scope="col" class="relative py-3.5 pl-3 pr-4 sm:pr-0">
                                <span class="sr-only">{{ __('reports.manager.compliance.details.headers.actions') }}</span>
                            </th>
                        </tr>
                    </x-slot>
