...
- [ ] 5.3 Run full test suite and formatting checks; fix regressions before completion. (Formatting completed via `vendor/bin/pint --dirty`; full suite currently fails on existing `PolicyRegistryIntegrationTest` final-class extension issue under `php artisan test --compact --parallel`.)
