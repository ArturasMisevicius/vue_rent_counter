<?php

return [
    'audit_log_action' => [
        'created' => 'Created',
        'updated' => 'Updated',
        'deleted' => 'Deleted',
        'archived' => 'Archived',
        'exported' => 'Exported',
        'approved' => 'Approved',
        'rejected' => 'Rejected',
        'restored' => 'Restored',
        'suspended' => 'Suspended',
        'reinstated' => 'Reinstated',
        'impersonated' => 'Impersonated',
        'logged_in' => 'Logged in',
        'logged_out' => 'Logged out',
        'sent' => 'Sent',
    ],
    'assignment_scope' => [
        'property' => 'Property',
        'building' => 'Building',
        'organization' => 'Organization',
        'tenant' => 'Tenant',
    ],
    'billing_frequency' => [
        'monthly' => 'Monthly',
        'quarterly' => 'Quarterly',
        'yearly' => 'Yearly',
        'one_time' => 'One time',
    ],
    'billing_method' => [
        'meter_based' => 'Meter based',
        'fixed_monthly' => 'Fixed monthly',
        'one_time' => 'One time',
        'manual' => 'Manual',
        'percentage' => 'Percentage',
        'formula_based' => 'Formula based',
        'included_free' => 'Included/free',
    ],
    'billing_readiness_status' => [
        'ready' => 'Ready',
        'warning' => 'Warning',
        'blocked' => 'Blocked',
        'not_configured' => 'Not configured',
    ],
    'distribution_method' => [
        'equal' => 'Equal',
        'area' => 'By area',
        'by_consumption' => 'By consumption',
        'by_occupancy' => 'By occupancy',
        'fixed_share' => 'Fixed share',
        'weighted_share' => 'Weighted share',
        'custom_formula' => 'Custom formula',
    ],
    'help_article_category' => [
        'getting_started' => 'Getting Started',
        'billing' => 'Billing',
        'readings' => 'Readings',
        'services' => 'Services',
        'invoices' => 'Invoices',
        'tenants' => 'Tenants',
        'contracts' => 'Contracts',
        'documents' => 'Documents',
        'troubleshooting' => 'Troubleshooting',
    ],
    'help_audience_role' => [
        'admin' => 'Admin',
        'manager' => 'Manager',
        'tenant' => 'Tenant',
        'superadmin' => 'Superadmin',
        'all' => 'All roles',
    ],
    'extra_charge_status' => [
        'draft' => 'Draft',
        'pending_review' => 'Pending review',
        'approved' => 'Approved',
        'included_in_invoice' => 'Included in invoice',
        'rejected' => 'Rejected',
        'cancelled' => 'Cancelled',
        'voided' => 'Voided',
    ],
    'extra_charge_type_code' => [
        'fixed_service' => 'Fixed service',
        'one_time_charge' => 'One-time charge',
        'manual_expense' => 'Manual expense',
        'penalty' => 'Penalty',
        'discount' => 'Discount',
        'rent' => 'Rent',
        'deposit' => 'Deposit',
        'correction' => 'Correction',
        'other' => 'Other',
    ],
    'integration_health_status' => [
        'healthy' => 'Healthy',
        'degraded' => 'Degraded',
        'failed' => 'Failed',
        'maintenance' => 'Maintenance',
        'unknown' => 'Unknown',
    ],
    'kyc_verification_status' => [
        'unverified' => 'Unverified',
        'pending' => 'Pending',
        'verified' => 'Verified',
        'rejected' => 'Rejected',
    ],
    'invoice_status' => [
        'draft' => 'Draft',
        'finalized' => 'Finalized',
        'partially_paid' => 'Partially paid',
        'paid' => 'Paid',
        'overdue' => 'Overdue',
        'void' => 'Void',
    ],
    'invoice_payment_status' => [
        'unpaid' => 'Unpaid',
        'partially_paid' => 'Partially paid',
        'paid' => 'Paid',
        'overdue' => 'Overdue',
        'overpaid' => 'Overpaid',
        'cancelled' => 'Cancelled',
        'voided' => 'Voided',
        'refunded' => 'Refunded',
    ],
    'language_status' => [
        'active' => 'Active',
        'inactive' => 'Inactive',
    ],
    'lead_import_batch_status' => [
        'previewed' => 'Previewed',
        'imported' => 'Imported',
        'failed' => 'Failed',
        'cancelled' => 'Cancelled',
    ],
    'lead_outreach_channel' => [
        'email' => 'Email',
        'phone' => 'Phone',
        'sms' => 'SMS',
        'whatsapp' => 'WhatsApp',
        'manual' => 'Manual',
        'other' => 'Other',
    ],
    'lead_outreach_direction' => [
        'outbound' => 'Outbound',
        'inbound' => 'Inbound',
        'internal_note' => 'Internal note',
    ],
    'lead_outreach_status' => [
        'draft' => 'Draft',
        'sent' => 'Sent',
        'received' => 'Received',
        'scheduled' => 'Scheduled',
        'due' => 'Due',
        'completed' => 'Completed',
        'cancelled' => 'Cancelled',
        'blocked' => 'Blocked',
    ],
    'lead_source_type' => [
        'aruodas_csv' => 'Aruodas CSV',
        'manual_import' => 'Manual import',
        'partner_list' => 'Partner list',
        'website_form' => 'Website form',
        'phone_inquiry' => 'Phone inquiry',
        'other' => 'Other',
    ],
    'listing_lead_status' => [
        'new' => 'New',
        'assigned' => 'Assigned',
        'contacted' => 'Contacted',
        'follow_up_needed' => 'Follow-up needed',
        'responded' => 'Responded',
        'interested' => 'Interested',
        'not_interested' => 'Not interested',
        'converted' => 'Converted',
        'duplicate' => 'Duplicate',
        'invalid' => 'Invalid',
        'do_not_contact' => 'Do not contact',
        'archived' => 'Archived',
    ],
    'invitation_status' => [
        'pending' => 'Pending',
        'accepted' => 'Accepted',
        'expired' => 'Expired',
        'revoked' => 'Revoked',
    ],
    'manager_membership_status' => [
        'invited' => 'Invited',
        'active' => 'Active',
        'disabled' => 'Disabled',
        'expired' => 'Expired',
    ],
    'meter_reading_submission_method' => [
        'admin_manual' => 'Admin manual',
        'tenant_portal' => 'Tenant portal',
        'mobile_app' => 'Mobile app',
        'api_integration' => 'API integration',
        'iot_gateway' => 'IoT gateway',
        'estimated' => 'Estimated',
        'import' => 'Import',
    ],
    'meter_reading_validation_status' => [
        'pending' => 'Pending',
        'valid' => 'Valid',
        'flagged' => 'Flagged',
        'rejected' => 'Rejected',
        'void' => 'Void',
    ],
    'meter_status' => [
        'active' => 'Active',
        'inactive' => 'Inactive',
        'maintenance' => 'Maintenance',
        'faulty' => 'Faulty',
        'retired' => 'Retired',
    ],
    'meter_type' => [
        'water' => 'Water',
        'water_cold' => 'Cold water',
        'water_hot' => 'Hot water',
        'electricity' => 'Electricity',
        'gas' => 'Gas',
        'heating' => 'Heating',
        'cooling' => 'Cooling',
        'steam' => 'Steam',
        'solar' => 'Solar',
        'custom' => 'Custom',
    ],
    'organization_status' => [
        'active' => 'Active',
        'pending' => 'Pending',
        'suspended' => 'Suspended',
        'cancelled' => 'Cancelled',
        'archived' => 'Archived',
    ],
    'payment_method' => [
        'bank_transfer' => 'Bank transfer',
        'direct_debit' => 'Direct debit',
        'card' => 'Card',
        'card_manual' => 'Manual card',
        'digital_wallet' => 'Digital wallet',
        'cheque' => 'Cheque',
        'cash' => 'Cash',
        'external_payment' => 'External payment',
        'other' => 'Other',
    ],
    'payment_status' => [
        'pending' => 'Pending',
        'confirmed' => 'Confirmed',
        'failed' => 'Failed',
        'cancelled' => 'Cancelled',
        'refunded' => 'Refunded',
        'partially_refunded' => 'Partially refunded',
        'voided' => 'Voided',
    ],
    'pricing_model' => [
        'fixed_monthly' => 'Fixed monthly',
        'fixed_daily' => 'Fixed daily',
        'consumption_based' => 'Consumption based',
        'tiered_rates' => 'Tiered rates',
        'hybrid' => 'Hybrid',
        'custom_formula' => 'Custom formula',
        'flat' => 'Flat',
        'time_of_use' => 'Time of use',
    ],
    'project_cost_record_type' => [
        'materials' => 'Materials',
        'contractor_fee' => 'Contractor fee',
        'permit' => 'Permit',
        'expense' => 'Expense',
        'other' => 'Other',
    ],
    'project_priority' => [
        'low' => 'Low',
        'medium' => 'Medium',
        'high' => 'High',
        'critical' => 'Critical',
    ],
    'project_status' => [
        'draft' => 'Draft',
        'planned' => 'Planned',
        'in_progress' => 'In progress',
        'on_hold' => 'On hold',
        'completed' => 'Completed',
        'cancelled' => 'Cancelled',
    ],
    'project_team_role' => [
        'manager' => 'Manager',
        'contributor' => 'Contributor',
        'observer' => 'Observer',
    ],
    'project_type' => [
        'maintenance' => 'Maintenance',
        'renovation' => 'Renovation',
        'inspection' => 'Inspection',
        'compliance' => 'Compliance',
        'installation' => 'Installation',
        'cleaning' => 'Cleaning',
        'emergency' => 'Emergency',
        'administrative' => 'Administrative',
        'other' => 'Other',
    ],
    'property_type' => [
        'apartment' => 'Apartment',
        'house' => 'House',
        'studio' => 'Studio',
        'office' => 'Office',
        'retail' => 'Retail',
        'warehouse' => 'Warehouse',
        'commercial' => 'Commercial',
        'industrial' => 'Industrial',
        'mixed_use' => 'Mixed use',
        'garage' => 'Garage',
        'parking' => 'Parking',
        'storage' => 'Storage',
        'other' => 'Other',
    ],
    'property_assignment_status' => [
        'scheduled' => 'Scheduled',
        'active' => 'Active',
        'move_out_scheduled' => 'Move-out scheduled',
        'ended' => 'Ended',
        'cancelled' => 'Cancelled',
    ],
    'property_occupancy_status' => [
        'vacant' => 'Vacant',
        'occupied' => 'Occupied',
        'move_in_scheduled' => 'Move-in scheduled',
        'move_out_scheduled' => 'Move-out scheduled',
        'unavailable' => 'Unavailable',
        'maintenance' => 'Maintenance',
    ],
    'move_out_process_status' => [
        'draft' => 'Draft',
        'scheduled' => 'Scheduled',
        'waiting_final_readings' => 'Waiting for final readings',
        'ready_for_final_invoice' => 'Ready for final invoice',
        'final_invoice_sent' => 'Final invoice sent',
        'completed' => 'Completed',
        'cancelled' => 'Cancelled',
    ],
    'portal_access_after_move_out' => [
        'keep_historical_access' => 'Keep historical access',
        'disable_immediately' => 'Disable immediately',
        'disable_after_final_invoice_paid' => 'Disable after final invoice is paid',
        'disable_after_retention_days' => 'Disable after retention period',
    ],
    'security_violation_severity' => [
        'low' => 'Low',
        'medium' => 'Medium',
        'high' => 'High',
        'critical' => 'Critical',
    ],
    'security_violation_type' => [
        'authentication' => 'Authentication',
        'authorization' => 'Authorization',
        'rate_limit' => 'Rate limit',
        'injection' => 'Injection',
        'csp' => 'CSP violation',
        'suspicious_ip' => 'Suspicious IP',
        'impersonation' => 'Impersonation',
        'tenant_isolation' => 'Tenant isolation',
        'data_access' => 'Data access',
        'data_export' => 'Data export',
        'session_hijack' => 'Session hijack',
    ],
    'service_type' => [
        'electricity' => 'Electricity',
        'water' => 'Water',
        'hot_water' => 'Hot water',
        'heating' => 'Heating',
        'gas' => 'Gas',
        'sewage' => 'Sewage',
        'cooling' => 'Cooling',
        'steam' => 'Steam',
        'solar' => 'Solar',
        'internet' => 'Internet',
        'maintenance' => 'Maintenance',
        'waste' => 'Waste',
    ],
    'service_configuration_status' => [
        'draft' => 'Draft',
        'active' => 'Active',
        'move_out_scheduled' => 'Move-out scheduled',
        'inactive' => 'Inactive',
        'archived' => 'Archived',
        'configuration_error' => 'Configuration error',
    ],
    'subscription_access_mode' => [
        'active' => 'Active',
        'limit_blocked' => 'Limit blocked',
        'grace_read_only' => 'Grace read only',
        'post_grace_read_only' => 'Post-grace read only',
        'suspended' => 'Suspended',
    ],
    'subscription_duration' => [
        'monthly' => 'Monthly',
        'quarterly' => 'Quarterly',
        'semi_annual' => 'Semi-annual',
        'yearly' => 'Yearly',
        'biennial' => 'Biennial',
        'triennial' => 'Triennial',
    ],
    'subscription_plan' => [
        'starter' => 'Starter',
        'basic' => 'Basic',
        'professional' => 'Professional',
        'enterprise' => 'Enterprise',
        'custom' => 'Custom',
    ],
    'tenant_status' => [
        'draft' => 'Draft',
        'active' => 'Active',
        'move_out_scheduled' => 'Move-out scheduled',
        'inactive' => 'Inactive',
        'moved_out' => 'Moved out',
        'archived' => 'Archived',
    ],
    'meter_reading_type' => [
        'regular' => 'Regular',
        'move_out_final' => 'Move-out final',
    ],
    'tenant_document_status' => [
        'draft' => 'Draft',
        'active' => 'Active',
        'pending_review' => 'Pending review',
        'verified' => 'Verified',
        'rejected' => 'Rejected',
        'expired' => 'Expired',
        'archived' => 'Archived',
        'deleted' => 'Deleted',
    ],
    'tenant_document_type' => [
        'invoice' => 'Invoice',
        'invoice_pdf' => 'Invoice PDF',
        'rental_contract' => 'Rental contract',
        'contract_appendix' => 'Contract appendix',
        'kyc_identity' => 'KYC identity',
        'kyc_address' => 'KYC address',
        'payment_receipt' => 'Payment receipt',
        'property_document' => 'Property document',
        'meter_photo' => 'Meter photo',
        'extra_charge_attachment' => 'Extra charge attachment',
        'repair_receipt' => 'Repair receipt',
        'provider_invoice' => 'Provider invoice',
        'other' => 'Other',
    ],
    'portal_access_status' => [
        'not_invited' => 'Not invited',
        'invited' => 'Invited',
        'invitation_expired' => 'Invitation expired',
        'active' => 'Active',
        'disabled' => 'Disabled',
    ],
    'invitation_status' => [
        'pending' => 'Pending',
        'accepted' => 'Accepted',
        'expired' => 'Expired',
        'revoked' => 'Revoked',
    ],
    'rental_contract_status' => [
        'draft' => 'Draft',
        'active' => 'Active',
        'expired' => 'Expired',
        'terminated' => 'Terminated',
        'renewed' => 'Renewed',
        'cancelled' => 'Cancelled',
    ],
    'unit_of_measurement' => [
        'm3' => 'm3',
        'kWh' => 'kWh',
        'MWh' => 'MWh',
        'day' => 'Day',
        'month' => 'Month',
        'collection' => 'Collection',
        'unit' => 'Unit',
    ],
    'subscription_status' => [
        'trialing' => 'Trialing',
        'active' => 'Active',
        'expired' => 'Expired',
        'suspended' => 'Suspended',
        'cancelled' => 'Cancelled',
    ],
    'system_setting_category' => [
        'general' => 'General',
        'billing' => 'Billing',
        'localization' => 'Localization',
        'security' => 'Security',
        'integrations' => 'Integrations',
        'notifications' => 'Notifications',
        'email' => 'Email',
        'subscription' => 'Subscription',
        'backups' => 'Backups',
        'maintenance' => 'Maintenance',
        'reporting' => 'Reporting',
        'api' => 'API',
        'compliance' => 'Compliance',
    ],
    'tariff_type' => [
        'flat' => 'Flat',
        'time_of_use' => 'Time of use',
        'seasonal' => 'Seasonal',
    ],
    'tariff_zone' => [
        'day' => 'Day',
        'night' => 'Night',
        'weekend' => 'Weekend',
        'peak' => 'Peak',
        'off_peak' => 'Off-peak',
        'shoulder' => 'Shoulder',
        'holiday' => 'Holiday',
        'super_off_peak' => 'Super off-peak',
    ],
    'user_role' => [
        'viewer' => 'Viewer',
        'superadmin' => 'Superadmin',
        'admin' => 'Admin',
        'manager' => 'Manager',
        'tenant' => 'Tenant',
    ],
    'user_status' => [
        'active' => 'Active',
        'inactive' => 'Inactive',
        'suspended' => 'Suspended',
    ],
    'weekend_logic' => [
        'apply_night_rate' => 'Apply night rate',
        'apply_day_rate' => 'Apply day rate',
        'apply_weekend_rate' => 'Apply weekend rate',
        'apply_off_peak_rate' => 'Apply off-peak rate',
        'apply_peak_rate' => 'Apply peak rate',
        'apply_shoulder_rate' => 'Apply shoulder rate',
        'apply_holiday_rate' => 'Apply holiday rate',
    ],
];
