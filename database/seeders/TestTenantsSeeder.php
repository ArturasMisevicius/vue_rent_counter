...
class TestTenantsSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        if ($tenant1Properties->count() > 0 && $tenantUser1) {
            Tenant::factory()
                ->forProperty($tenant1Properties[0])
                ->create([
                    'name' => $tenantUser1->name,
                    'email' => $tenantUser1->email,
                    'phone' => '+370 600 12345',
                    'lease_start' => Carbon::now()->subMonths(14),
        ...
        if ($tenant1Properties->count() > 1 && $tenantUser2) {
            Tenant::factory()
                ->forProperty($tenant1Properties[1])
                ->create([
                    'name' => $tenantUser2->name,
                    'email' => $tenantUser2->email,
                    'phone' => '+370 600 23456',
                    'lease_start' => Carbon::now()->subMonths(16),
        ...
        if ($tenant2Properties->count() > 0 && $tenantUser3) {
            Tenant::factory()
                ->forProperty($tenant2Properties[0])
                ->create([
                    'name' => $tenantUser3->name,
                    'email' => $tenantUser3->email,
                    'phone' => '+370 600 34567',
                    'lease_start' => Carbon::now()->subMonths(15),
        ...
        if ($tenant2Properties->count() > 1) {
            Tenant::factory()
                ->forProperty($tenant2Properties[1])
                ->create([
                    'name' => 'Jonas Petraitis',
                    'email' => 'jonas.petraitis@example.com',
                    'phone' => '+370 600 45678',
                    'lease_start' => Carbon::now()->subMonths(12),
