...
- **Tests**: Pest + PHPUnit suites (Feature, Unit, Filament folders) plus property tests inside `tests/Feature/*PropertyTest.php` that enforce tenant isolation, tariff accuracy, meter reading validation, and authorization.
...
- **Billing integrity**: Every invoice must snapshot tariffs and include audited meter reading snapshots; `MeterReadingObserver` recalculations should not re-open finalized invoices.
...
- **Backend**: Keep services composable (`BillingService`, `TariffResolver`, `AccountManagementService`); prefer FormRequests for validation and policies for every mutating action; guard `tenant_id` via `TenantScope`.
